...
class Dispatch2Iq(val schdBlockParams : SchdBlockParams)(implicit p: Parameters) extends LazyModule with HasXSParameter {
  ...
  lazy val module: Dispatch2IqImp = schdBlockParams.schdType match {
    case IntScheduler() => new Dispatch2IqInt2Imp(this)(p, schdBlockParams)
...
class Dispatch2IqInt2Imp(override val wrapper: Dispatch2Iq)(implicit p: Parameters, params: SchdBlockParams)
  ...
  //int dq1
  private val numIQ = 2
  val dq1IQParams = params.issueBlockParams.drop(2)
  val exeUnitParms = dq1IQParams.map(_.exuBlockParams)
  val exeFuConfigs = dq1IQParams.map(_.exuBlockParams.map(_.fuConfigs)).flatten
  val IQFuConfigs = dq1IQParams.map(_.exuBlockParams.map(_.fuConfigs).flatten)
  val allFuConfigs = dq1IQParams.map(_.exuBlockParams.map(_.fuConfigs)).flatten.flatten.distinct
  val fuConfigMapIQ = allFuConfigs.map{ case x =>
    x -> IQFuConfigs.map(_.contains(x))
  }
  val fuConfigsDq1 = fuConfigMapIQ.filter(x => x._2.count(_ == true) == 1).map(_._1)
  println(s"[Dispatch2IqIntImp] IQFuConfigs: ${IQFuConfigs.map(_.map(_.name))}")
  println(s"[Dispatch2IqIntImp] allFuConfigs: ${allFuConfigs.map(_.name)}")
  println(s"[Dispatch2IqIntImp] fuConfigMapIQ: ${fuConfigMapIQ.map(x => (x._1.name, x._2))}")
  println(s"[Dispatch2IqIntImp] fuConfigsDq1: ${fuConfigsDq1.map(_.name)}")
  val uopsInFuType = VecInit(uopsInDq1.map(x => Mux(x.valid, x.bits.fuType, 0.U.asTypeOf(x.bits.fuType))))
  val inFuTypeIsOnlyOneIQ  = VecInit(uopsInFuType.map { case x =>
    VecInit(fuConfigsDq1.map { case y => x(y.fuType.id) })
  })
  ...
  uopsInDq1.zip(uopsInReady).map{ case (u, r) =>
    u.ready := r
  }
  ...
  io.out.drop(2).zipWithIndex.map{ case (iq, iqIndex) =>
    iq.zipWithIndex.map{ case (enq, i) =>
      if (i == 0) {
        enq.valid := PriorityMuxDefault(popSelIQOHIsOne.map(_(iqIndex)).zip(uopsInDq1.map(_.valid)), false.B)
        enq.bits := PriorityMuxDefault(popSelIQOHIsOne.map(_(iqIndex)).zip(uopsInDq1.map(_.bits)), 0.U.asTypeOf(enq.bits))
      }
      else if (i == 1) {
        enq.valid := PriorityMuxDefault(popSelIQOHIsTwo.map(_(iqIndex)).zip(uopsInDq1.map(_.valid)), false.B)
        enq.bits := PriorityMuxDefault(popSelIQOHIsTwo.map(_(iqIndex)).zip(uopsInDq1.map(_.bits)), 0.U.asTypeOf(enq.bits))
      }
      else {
