...
class Sbuffer(implicit p: Parameters)
  ...
  val io = IO(new Bundle() {
    ...
    val vecDifftestInfo = Vec(EnsbufferWidth, Flipped(Decoupled(new DiffStoreIO)))
//    val vecDifftestInfo = Input(new DiffStoreIO)
  ...
  */
  ...
  if (env.EnableDifftest) {
    ...
    for (i <- 0 until EnsbufferWidth) {
      ...
      val uop = io.vecDifftestInfo(i).bits.diffInfo

      val unaligned_start = io.vecDifftestInfo(i).bits.diffInfo.start
      val unaligned_offset = io.vecDifftestInfo(i).bits.diffInfo.offset
      val unaligned_start_bits = (io.vecDifftestInfo(i).bits.diffInfo.start << 3.U).asUInt
      val unaligned_offset_bits = (io.vecDifftestInfo(i).bits.diffInfo.offset << 3.U).asUInt
      ...
      val EEB = (1.U << eew).asUInt //Only when VLEN=128 effective element byte
      val EEWBits = (EEB << 3.U).asUInt
      val nf = Mux(isVsr, 0.U, uop.nf)

      val isSegment = nf =/= 0.U && !isVsm
      val isVSLine = (isVse || isVsm || isVsr) && !isSegment
      val isWline = io.vecDifftestInfo(i).bits.pmaStore.bits.wline
      ...
      val rawData = io.vecDifftestInfo(i).bits.pmaStore.bits.data
      val rawMask = io.vecDifftestInfo(i).bits.pmaStore.bits.mask
      val rawAddr = io.vecDifftestInfo(i).bits.pmaStore.bits.addr
      ...
      when(isVSLine) {
        ...
        val storeCommit = io.vecDifftestInfo(i).bits.pmaStore.fire && splitMask.orR && io.vecDifftestInfo(i).bits.pmaStore.bits.vecValid
        // align with ref
        ...
        difftestCommon.data := wdata
        difftestCommon.mask := wmask
        difftestCommon.robidx := io.vecDifftestInfo(i).bits.diffInfo.robIdx.value
        difftestCommon.pc := io.vecDifftestInfo(i).bits.diffInfo.pc

      }.elsewhen(!isWline) {
        val storeCommit = io.vecDifftestInfo(i).bits.pmaStore.fire
        val waddr = ZeroExt(Cat(io.vecDifftestInfo(i).bits.pmaStore.bits.addr(PAddrBits - 1, 3), 0.U(3.W)), 64)
        val sbufferMask = shiftMaskToLow(io.vecDifftestInfo(i).bits.pmaStore.bits.addr, io.vecDifftestInfo(i).bits.pmaStore.bits.mask)
        val sbufferData = shiftDataToLow(io.vecDifftestInfo(i).bits.pmaStore.bits.addr, io.vecDifftestInfo(i).bits.pmaStore.bits.data)
        val wmask = sbufferMask
        val wdata = sbufferData & MaskExpand(sbufferMask)

        difftestCommon.coreid := io.hartId
        difftestCommon.index := (i * VecMemFLOWMaxNumber).U
        difftestCommon.valid := storeCommit && io.vecDifftestInfo(i).bits.pmaStore.bits.vecValid
        difftestCommon.addr := waddr
        difftestCommon.data := wdata
        difftestCommon.mask := wmask
        difftestCommon.robidx := io.vecDifftestInfo(i).bits.diffInfo.robIdx.value
      ...
      for (index <- 0 until WlineMaxNumber) {
        ...
        val storeCommit = io.vecDifftestInfo(i).bits.pmaStore.fire && io.vecDifftestInfo(i).bits.pmaStore.bits.vecValid
        val blockAddr = get_block_addr(io.vecDifftestInfo(i).bits.pmaStore.bits.addr)
        ...
        when(isWline) {
          ...
          difftest.valid := storeCommit
          difftest.addr := blockAddr + (index.U << wordOffBits)
          difftest.data := io.vecDifftestInfo(i).bits.pmaStore.bits.data
          difftest.mask := ((1 << wordBytes) - 1).U
          difftest.robidx := io.vecDifftestInfo(i).bits.diffInfo.robIdx.value
          difftest.pc := io.vecDifftestInfo(i).bits.diffInfo.pc

          assert(!storeCommit || (io.vecDifftestInfo(i).bits.pmaStore.bits.data === 0.U), "wline only supports whole zero write now")
        }
      }
