    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ExeUnitParams("LDU0", Seq(LduCfg),
                              Seq(IntWB(5, 0), VfWB(7, 0)),
                              Seq(Seq(IntRD(8, 0))), true, 2),
      ), numEntries = 16, numEnq = 1, numComp = 15),
      IssueBlockParams(Seq(
        ExeUnitParams("LDU1", Seq(LduCfg), Seq(IntWB(6, 0), VfWB(8, 0)), Seq(Seq(IntRD(9, 0))), true, 2),
      ), numEntries = 16, numEnq = 1, numComp = 15),
      IssueBlockParams(Seq(
        ExeUnitParams("VLSU0", Seq(VlduCfg, VstuCfg, VseglduSeg, VsegstuCfg), Seq(VfWB(9, 0), V0WB(7, 0), VlWB(port = 2, 0)), Seq(Seq(VfRD(18, 0)), Seq(VfRD(19, 0)), Seq(VfRD(20, 0)), Seq(V0RD(6, 0)), Seq(VlRD(6, 0)))),
      ), numEntries = 16, numEnq = 2, numComp = 14),
      IssueBlockParams(Seq(
        ExeUnitParams("STD0", Seq(StdCfg, MoudCfg), Seq(), Seq(Seq(IntRD(11, 0), VfRD(21, 0)))),
      ), numEntries = 20, numEnq = 2, numComp = 18),
    ),
