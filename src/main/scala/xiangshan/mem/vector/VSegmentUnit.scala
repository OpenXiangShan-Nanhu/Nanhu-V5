...
import xs.utils.{LookupTree, ZeroExt}
import xs.utils.perf.XSError
import xs.utils.queue.CircularQueuePtr
...
//      instMicroOp.exceptionVaddr    := io.dtlb.resp.bits.fullva //todo
  ...
  when(
    (state === s_latch_and_merge_data || state === s_misalign_merge_data) &&
      io.rdcache.resp.bits.error_delayed && RegNext(io.csrCtrl.cache_error_enable) &&
      segmentActive
  ) {
    exception_pa := true.B
