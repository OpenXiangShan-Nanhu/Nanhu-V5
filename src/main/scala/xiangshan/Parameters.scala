...
(
  ...
  val fpSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      // ), numEntries = 18, numEnq = 2, numComp = 16),
      IssueBlockParams(Seq(
        ExeUnitParams("FEX3", Seq(FdivCfg), Seq(FpWB(port = 4, 1)), Seq(Seq(FpRD(2, 1)), Seq(FpRD(5, 1)))),
        ExeUnitParams("FEX4", Seq(FdivCfg), Seq(FpWB(port = 3, 0)), Seq(Seq(FpRD(6, 1)), Seq(FpRD(7, 1)))),
