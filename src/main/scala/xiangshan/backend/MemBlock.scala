...
class MemBlockInlined()(implicit p: Parameters) extends LazyModule
  ...
  val mmu = LazyModule(new MMUWrapper)
  ...
  with SdtrigExt
{
  val io = IO(new Bundle {
    ...
    val frontendMMUInfo = Flipped(new frontendMMUBundle)
...
//  val wbRouter = Module(new MemBlockWriteBackRouter)
  ...
  val dtlb_reqs = mmu.io.memblock.tlb

  val pmpCheckResp = mmu.io.memblock.pmpCheckResp
  for (i <- 0 until LduCnt) {
  ...
  for (i <- 0 until LduCnt) {
    ...
    loadUnits(i).io.tlb_hint.id := mmu.io.memblock.lduHint.req(i).id
    loadUnits(i).io.tlb_hint.full := mmu.io.memblock.lduHint.req(i).full
    ...
    lsq.io.tlb_hint <> mmu.io.memblock.lqHint
  ...
  for (i <- 0 until StaCnt) {
    ...
    // dtlb
    stu.io.tlb          <> dtlb_reqs(DTLB_PORT_START_ST + i)
  ...
  if (env.EnableDifftest) {
    ...
    sbuffer.io.vecDifftestInfo .zipWithIndex.map{ case (sbufferPort, index) =>
      if (index == 0) {
        ...
        sbufferPort.bits.pmaStore.valid  := Mux(vSegmentDifftestValid, vSegmentUnit.io.sbuffer.fire, lsq.io.sbufferVecDifftestInfo(0).bits.pmaStore.valid)
        sbufferPort.bits.pmaStore.bits  := Mux(vSegmentDifftestValid, vSegmentUnit.io.sbuffer.bits, lsq.io.sbufferVecDifftestInfo(0).bits.pmaStore.bits)
        sbufferPort.bits.diffInfo  := Mux(vSegmentDifftestValid, vSegmentUnit.io.vecDifftestInfo.bits, lsq.io.sbufferVecDifftestInfo(0).bits.diffInfo)
  ...
  val amoTlb = dtlb_reqs(DTLB_PORT_START_LD)
  ...
  lsq.io.debugTopDown.robHeadMissInDTlb := DontCare //todo
...
//  val perfFromPTW   = ptw.getPerfEvents
  ...
  val allPerfEvents = perfFromUnits ++ perfBlock
